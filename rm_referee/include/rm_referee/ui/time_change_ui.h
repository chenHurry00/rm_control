...
public:
    ...
    : TimeChangeGroupUi(rpc_value, base, "balance_pitch")
  {
    ...
    config["start_position"][0] = triangle_left_point_[0];
    config["start_position"][1] = triangle_left_point_[1];
    config["end_position"][0] = triangle_right_point_[0];
    config["end_position"][1] = triangle_right_point_[1];
    graph_vector_.insert(std::make_pair<std::string, Graph*>("triangle_top_side", new Graph(config, base_, id_++)));
  }
